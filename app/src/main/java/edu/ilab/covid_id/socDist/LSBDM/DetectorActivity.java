...
package edu.ilab.covid_id.socDist.LSBDM;
...
public class DetectorActivity extends CameraActivity implements OnImageAvailableListener {
  ...
  private static final String TF_OD_API_MODEL_FILE = "LSBDMdetect.tflite";   //name of input file for MODEL must be tflite format
                                                                        //TIP: if creating subclass for say mask detection make your detector
                                                                        //   file called maskdetect.flite and put in assets folder
  private static final String TF_OD_API_LABELS_FILE = "file:///android_asset/LSBDMlabelmap.txt";  //LabelMap file listed classes--same order as training
              ...
              if (location != null && result.getConfidence() >= minimumConfidence) { //ONLY display if the result has a confidence > threshold
                ...
                if(CovidRecord.readyStoreRecord(MapsActivity.covidRecordLastStoreTimestamp, MapsActivity.deltaCovidRecordStoreTimeMS, MapsActivity.covidRecordLastStoreLocation, MapsActivity.currentLocation, MapsActivity.deltaCovidRecordStoreLocationM)) {
                  ...
                  if(title.contains("Bad")) { // needs 'contains' method instead of "==" because title has /n nwline char at end of string
                    ...
                    LOGGER.i("-----Bad-----",riskThresholdHigh_SocDist_LSBDM);
                    ...
                    //based on both confidence value will set risk in range
                  LOGGER.i("-----Good-----",riskThresholdCaution_SocDist_LSBDM);
